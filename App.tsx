...
import { fetchWorkshopData } from './services/trelloService';
...
const App: React.FC = () => {
  ...
  const loadData = async () => {
    try {
      ...
      // Filtra apenas estágios válidos e ordena por prioridade
  ...
  useEffect(() => {
    loadData();
    const refreshInterval = setInterval(loadData, 30000); // Atualiza a cada 30 segundos
  ...
  useEffect(() => {
    ...
    }, 10000); // Muda de página a cada 10 segundos
        ...
        <div className="flex items-center gap-4">
          <div className="flex items-center justify-center">
            <img 
              ...
              onError={(e) => {
                e.currentTarget.parentElement!.innerHTML = `<div class="w-10 h-10 bg-white rounded-xl flex items-center justify-center"><svg viewBox="0 0 24 24" class="w-6 h-6 text-black fill-current"><path d="M18.92 6.01C18.72 5.42 18.16 5 17.5 5h-11c-.66 0-1.21.42-1.42 1.01L3 12v8c0 .55.45 1 1 1h1c.55 0 1-.45 1-1v-1h12v1c0 .55.45 1 1 1h1c.55 0 1-.45 1-1v-8l-2.08-5.99zM6.5 16c-.83 0-1.5-.67-1.5-1.5S5.67 13 6.5 13s1.5.67 1.5 1.5S7.33 16 6.5 16zm11 0c-.83 0-1.5-.67-1.5-1.5s.67-1.5 1.5-1.5 1.5.67 1.5 1.5-.67 1.5-1.5 1.5zM5 11l1.5-4.5h11L19 11H5z" /></svg></div>`;
              }}
            />
          </div>
